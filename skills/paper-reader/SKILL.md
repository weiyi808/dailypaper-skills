...
description: |
  ...
  **高优先触发（避免漏调）**:
  - 只要用户输入中同时出现「读一下/帮我读/分析这篇/总结这篇」和「.pdf 或 PDF 文件名」，必须优先调用本 skill
  - 示例："读一下 InstaLoc.pdf"、"帮我读 这篇 xxx.pdf"、"分析这篇 paper.pdf"
  - 即使用户未给绝对路径，也先进入本 skill，先做路径定位再继续
context: fork
...
## 0.5 最小执行协议（防止“看起来调用了但没跑流程”）

命中触发词后，必须按顺序完成以下动作，不可跳步：

1. 明确回显当前输入类型（PDF 路径 / PDF 文件名 / arXiv / Zotero）
2. 若是 PDF 文件名但非绝对路径：先在常见目录与工作区搜索定位文件
3. 成功读取论文源（Read/WebFetch）后再产出内容；若读取失败必须明确报错并给出下一步
4. 输出末尾必须询问是否继续执行（深入解释 / 对比相关工作 / 保存到 Obsidian）

若第 2-3 步失败，不得直接给“像是读完了”的总结。
...
5. **图片在线优先（默认）**: 默认仅使用可访问的在线图片链接（arXiv HTML / IEEE Xplore / 项目主页 / GitHub），**不要自动下载到本地**；仅当用户明确要求离线化时才本地下载

> 公式/图片/表格的详细质量规范见 `references/quality-standards.md`

### 图片获取流程（多源 fallback，默认在线模式）
...
5. **来源 D — PDF 提取**（仅在在线来源不足，且用户明确要求补全/离线时启用）：
   - `pdfimages -png` 从 PDF 中提取，筛选 >10KB 的有效图片
6. 笔记中优先用 `![Figure X](url)` 外链嵌入（默认）
7. 验证：外链可加载（HTTP 200 或页面可正常渲染）；仅在本地化模式下再检查本地文件 >10KB
...
### 图片可靠性保障（默认不下载，仅做在线可达检查）

笔记保存后，默认只做“在线图片可达性”检查，不自动下载：
- 可达外链：保持不动
- 不可达外链：优先尝试替换为其他在线来源（arXiv HTML / IEEE / 项目主页 / GitHub）
- 若仍不可达：在图后标注「链接失效待修复」，并在结尾汇总失效链接

仅在用户明确要求“离线可见/本地化图片”时，才执行：
```bash
python3 ../daily-papers/download_note_images.py "{笔记完整路径}"
```
- 本地化后再将失败外链替换为 `assets/`，并把 frontmatter `image_source` 更新为 `mixed` 或 `local`
...
1. 只有在 `AUTO_REFRESH_INDEXES=true` 时才刷新目录页：
   ...
   ```
2. 图片策略同步：
   - 默认在线模式：`image_source: online`
   - 用户要求本地化且执行成功后：`image_source: mixed` 或 `local`
...
## 5.5 文献树、挑战洞察与 SLAM 知识库联动（每篇论文必做；批量可读性见下）

在保存单篇笔记后，必须更新 `{NOTES_PATH}` 下与 SLAM 研究相关的**总览文件**（路径相对 `论文笔记` 根，与 `paper_notes_folder` 一致）：

| 文件 | 作用 |
|------|------|
| `SLAM-文献树.md` | 按技术路线 + 创新性等级（S/A/B/C）归档，带 `[[论文笔记名]]` |
| `SLAM-挑战洞察树.md` | Challenge → 可复用洞察 + 代表工作链接 |
| `SLAM研究知识库.md` | 领域总览与入口：维护指向双树的链接，并可增「近期入库/专题」索引（不必重复粘贴全文） |

规则（每篇至少落实一次，写入上述**至少一个**总览文件；推荐三处都挂链，避免只在 `_待整理` 落笔记而总览空白）：

1. 将论文归入至少一个「技术路线」节点（激光 SLAM / 视觉 SLAM / 多传感器 / 语义建图 / 神经隐式建图）
2. 给出创新性等级（S/A/B/C）与一句理由
3. 将论文映射到至少一个挑战节点，并补一句可复用洞察
4. 若论文改变了研究优先级，更新 `Milestone Tasks` 勾选项或新增任务

### 与 `folder-papers-reader`（目录批量）的配合（重要）

- **根因说明**：目录批量往往「多会话/子任务」执行，若单篇流程**未真正写入**上述总览文件，或 agent 为省 token **跳过了 5.5**，用户会看到「`_待整理` 有笔记、但总览像没更新」。
- **允许策略（二选一，禁止全跳过）**：
  - **策略 A（推荐单篇）**：每保存一篇笔记，立即追加/更新三份总览中的对应条目（最符合本节「每篇必做」）。
  - **策略 B（批量合并）**：单篇仅保证笔记与概念库；**同一批目录任务全部结束后**，由 **`folder-papers-reader` 的收尾步骤**一次性把本批新笔记合并进三份总览（去重、归类、补挑战洞察）。若采用策略 B，**单篇结束时仍须在笔记内写好可粘贴进树里的「一行归档句」**（方法名 + 一句贡献），便于批量合并。
- **铁律**：策略 A 与策略 B **至少执行其一**；不得出现「笔记已生成但三份总览完全未改」的交付。
...
- [ ] 正文中技术术语有 `[[概念]]` 内联链接？
- [ ] 概念库已更新（缺失的概念已创建）？
- [ ] 图片可用（默认：外链可加载；仅本地化模式：本地 >10KB）？
- [ ] **5.5** 已落实：`SLAM-文献树.md` / `SLAM-挑战洞察树.md` / `SLAM研究知识库.md` 至少其一已出现本篇的 `[[链接]]` 或专题索引（目录批量若采用策略 B，则须确认将由 `folder-papers-reader` 收尾合并）？

## 7. 执行策略（默认静默落地）

默认不向用户二次确认，直接执行完整流程并落地：
- 直接保存到 Obsidian
- 直接更新缺失概念笔记与**三份 SLAM 总览**（见 5.5；目录批量可与 `folder-papers-reader` 分工）
- 图片默认在线模式（不下载），仅当用户明确要求离线化才本地化

完成后仅返回简短结果（保存路径 + 关键统计），无需展示完整中间过程。
...
每篇仍须遵守 **5.5**（或明确由调用方在整批结束后合并总览，见 5.5「策略 B」）。

### 目录批量（`folder-papers-reader`）

若入口是「读文件夹论文」，**必须由 `folder-papers-reader` 在全部尝试处理完成后**执行：

1. 刷新 MOC（`generate_concept_mocs.py` / `generate_paper_mocs.py`）
2. **合并更新** `SLAM-文献树.md`、`SLAM-挑战洞察树.md`、`SLAM研究知识库.md`（当本批与 SLAM/退化建图相关时；若用户目录明显非 SLAM，可在 `SLAM研究知识库` 仅增加一句说明与指向 `_待整理` 的链接，避免误归档）

单篇 `paper-reader` 与目录批量 **不得互相推诿**导致三份总览始终不更新。
