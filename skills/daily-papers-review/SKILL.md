...
为避免“抓取很宽、点评很严导致清空”，点评阶段必须执行与抓取同源的两阶段筛选：

1. **阶段 A：沿用抓取结果，不重复发明新标准**
   - 输入候选必须来自 `/tmp/daily_papers_enriched.json`（即 `fetch_and_score.py` 的输出经富化后的集合）
   - 先按 `score` 降序处理，保留抓取阶段的相关性排序
   - 不允许仅凭主观印象把高分候选整批清空

2. **阶段 B：SLAM 主线重排（不是重置）**
   - 在阶段 A 候选中，用以下主线关键词做“优先级重排”（命中越多优先级越高）：
     - `slam`, `localization`, `mapping`, `odometry`, `loop closure`,
       `place recognition`, `lidar`, `rgb-d`, `visual-inertial`, `vslam`
   - 同时使用以下排除词降低优先级或剔除：
     - `agent`, `llm`, `gui agent`, `voice`, `audio`, `cybersecurity`,
       `video editing`, `scientist`, `law`
   - 若标题/摘要同时命中主线词与排除词，必须在“被排除的论文”中写明原因，禁止静默丢弃

3. **最低保留原则**
   - 只要候选池中存在 >=1 篇明显属于 SLAM/建图主线的论文，就**必须**进入分流表（至少放在“值得看”）
   - 只有在全量候选均不满足主线时，才允许输出“必读/值得看为空”

4. **可追溯性要求**
   - 在开头锐评里必须简述：本次主线命中论文数量、被排除的大类原因（例如“Agent/LLM 偏多”）
   - “被排除的论文”至少列出标题 + 1 句具体原因，避免只写“无关”
